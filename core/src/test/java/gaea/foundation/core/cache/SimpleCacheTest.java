...
import gaea.foundation.core.cache.simplecache.SimpleCache;
...
public class SimpleCacheTest {
    protected Cache firstCache = null;
    protected Cache secondCache = null;

    @Before
    public void init() {
        firstCache = getCache("Group_01", false);
        secondCache = getCache("Group_02", false);
    ...
    public void testGetPutBatch() {
        final Cache firstCache = getCache("Group_01", false);
        final Cache secondCache = getCache("Group_02", true);
        firstCache.put("cache_01", 100);
        firstCache.put("cache_02", "abc", 1); //1秒后过期
        firstCache.put("cache_03", "edf");    //
