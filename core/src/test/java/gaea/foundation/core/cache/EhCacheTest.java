...
import gaea.foundation.core.cache.ehcache.EhCacheProvider;
...
public class EhCacheTest {
    ...
    public void testGetKeys() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                Assert.assertEquals("再次取得Key列表，键列表返回出错", firstCache.getKeys().size(), 3);
                Assert.assertEquals("取得允许空缓存的Key列表，键列表返回出错", secondCache.getKeys().size(), 4);
            }
        }).start();
        sleepThread(1100);
        new Thread(new Runnable() {
            @Override
            public void run() {
                Assert.assertEquals("取得已过期的Key列表，键列表返回出错", firstCache.getKeys().size(), 3);
                sleepThread(300);
    ...
    public Cache getCache(String cacheName, boolean emptySupport) {
        return EhCacheProvider.createProvider().createCache(cacheName, emptySupport);
    }
}
